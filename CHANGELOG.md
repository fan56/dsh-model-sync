...
- **One-time legacy settings import (pre-0.1.7 → 0.1.7 upgrade path).** The dsh 0.1.7 host imports the removed `settings.yaml` once by "section name = entry id" and renames it `settings.yaml.imported`; installs where that one-shot import never carried the legacy `model-sync:` section over (host skipped the rename, manually restored file, plugin installed after the host upgrade) got their config silently stranded. The plugin now recovers it at boot (`src/legacy-import.ts`): it reads `settings.yaml.imported` (falling back to `settings.yaml`), flat-parses the `model-sync:` section with a conservative scalar-only parser (quoted values unquoted, single-line quoted flow JSON parsed, nested blocks/folded values skipped), keeps only keys whose legacy value differs from the current effective value (read through the volatile refs), and writes them through `settings.update('dsh-model-sync', …)`. Success lands a persistent audit record at `~/.dsh/storages/dsh-model-sync/legacy-import.json` (`{at, outcome, source, imported, skipped}`, outcome one of `imported`/`no-op`/`no-legacy`/`no-section`); the marker's presence skips the whole migration (idempotent — later edits of the imported file cannot resurrect old values), while a failed `update` writes no marker and retries at the next boot. The whole flow is awaited at the tail of `apply()` under a full try/catch: any failure only warns, plugin activation never depends on it. +19 tests (`test/legacy-import.test.mjs`).
