// Plan C (settings-seam): one-time legacy settings import for the 0.1.5 → 0.1.7 upgrade.

/**
 * The dsh 0.1.7 host renames the old `~/.dsh/settings.yaml` to
 * `settings.yaml.imported` and imports its sections into the active profile
 * keyed by "section name = entry id". This plugin's old section was named
 * `model-sync` — which HAPPENS to equal the entry id (`OWN_ENTRY_ID` in
 * index.ts), so the host's one-shot import usually already carried the values
 * over. But installs where the import never fired (host skipped the rename,
 * a manually restored settings.yaml, plugin installed after the host
 * upgrade) still hold their config in the legacy section. This module
 * recovers it once:
 *
 *   1. read `<home>/settings.yaml.imported` (preferred) →
 *      `<home>/settings.yaml` (fallback);
 *   2. flat-parse the `model-sync:` section (scalars only, conservative);
 *   3. keep keys whose legacy value differs from the current effective value;
 *   4. `settings.update('dsh-model-sync', patch)` → on success write the
 *      audit marker `<home>/storages/dsh-model-sync/legacy-import.json`;
 ...
 *
 * Deliberately a standalone copy of the same contract dsh-topics-memory
 * implements: the two plugins share no runtime, only the convention.
 *
 * @module dsh-model-sync/legacy-import
 */
...
/** Profile entry id the recovered values are written to (index.ts OWN_ENTRY_ID). */
export const ENTRY_ID = 'dsh-model-sync'

/** Legacy top-level section name in the old settings.yaml. */
export const LEGACY_SECTION = 'model-sync'

/** Storage dir name for the audit marker (repo dir name, host convention). */
export const STORAGE_DIR_NAME = 'dsh-model-sync'

/** Legacy keys worth recovering, mapped 1:1 onto Config keys of the same name. */
export const LEGACY_KEYS = ['writeMode', 'intervalMinutes'] as const
...
export interface LegacyImportInput {
  /** Resolved dsh home (`resolveDshHome()` at the call site). */
  home: string
...
export type LegacyImportOutcome =
  ...
  | 'no-section' // legacy file exists without a `model-sync:` section, marker written
...
/** Dsh home resolution: `$DSH_HOME` when set, `~/.dsh` otherwise (the same
 *  shape remote-catalog.ts bakes in for the models-store path). */
